...
def test_agent_backend_framework_uses_read_file():
    ...
    # Should use read_file tool to check source code
    tool_names = [tc["tool"] for tc in data["tool_calls"]]
    assert "read_file" in tool_names, "Expected read_file to be called"


@pytest.mark.skipif(
    not os.getenv("LLM_API_KEY") and not os.getenv("LMS_API_KEY"),
    reason="LLM_API_KEY or LMS_API_KEY is not set; skipping LLM integration test",
)
def test_agent_database_items_uses_query_api():
